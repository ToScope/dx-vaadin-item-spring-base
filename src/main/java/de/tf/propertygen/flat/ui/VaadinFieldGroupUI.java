...
import de.tf.propertygen.flat.presenter.QuotePresenterVaadinFieldGroup;
import de.tf.propertygen.util.PrintingPropertyChangeListener;

@Theme("ditem_property_gen_base")
@PreserveOnRefresh
@SpringUI()
public class VaadinFieldGroupUI extends AbstractStartUI {
	private static final long serialVersionUID = 1L;

	@Override
	protected void bind(QuoteUI ui) {
		Quote quoteModel = new SampleQuote();
		quoteModel.add(new PrintingPropertyChangeListener());
		new QuotePresenterVaadinFieldGroup(ui, quoteModel);
	}
